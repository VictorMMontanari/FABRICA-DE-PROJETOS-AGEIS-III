...
utilizando o modelo e o scaler treinados em projeto9_v2.ipynb
"""
...
def carregar_modelo_e_scaler():
    """Carrega o modelo e o scaler treinados dos arquivos .joblib"""
    modelo_path = Path.cwd() / "projeto9_v2.joblib"
    scaler_path = Path.cwd() / "scaler_projeto9.joblib"

    if not modelo_path.exists():
        st.error(f"❌ Modelo não encontrado em {modelo_path}")
        st.info(
            "Por favor, execute o notebook `projeto9_v2.ipynb` para treinar e exportar o modelo."
        )
        st.stop()

    if not scaler_path.exists():
        st.error(f"❌ Scaler não encontrado em {scaler_path}")
        st.info(
            "Por favor, salve o scaler no seu notebook usando: joblib.dump(scaler, 'scaler_projeto9.joblib')"
        )
        st.stop()
...
def criar_features_a_partir_de_datas(data_abertura, data_fechamento):
    ...
    features_temp["mes_sin"] = np.sin(
        2 * np.pi * features_temp["mes_abertura"] / 12
    )
    features_temp["mes_cos"] = np.cos(
        2 * np.pi * features_temp["mes_abertura"] / 12
    )
...
def processar_entrada_usuario(
    ...
    """Processa os dados de entrada do usuário e cria um DataFrame com as features na

    ordem exata esperada pelo modelo.
    """

    # Features temporais
    features_temp = criar_features_a_partir_de_datas(
...
def normalizar_features(df_features, scaler):
    """Normaliza as features usando o StandardScaler importado do treinamento
    ...
    # Correção Crítica: Usando .transform() com o scaler do treino para não zerar a variância
    df_features_norm[scale_cols] = scaler.transform(df_features[scale_cols])
...
with st.sidebar:
    ...
    st.info("""
        ...
        **Arquivos Utilizados:**
        - `projeto9_v2.joblib` (Modelo)
        - `projeto9_v2.joblib (Scaler)
    """)
...
if st.button("🚀 Fazer Predição", type="primary", use_container_width=True):
    ...
    with st.spinner("Carregando modelo e componentes de transformação..."):
        modelo, scaler = carregar_modelo_e_scaler()

    # Processar dados
    with st.spinner("Processando e transformando dados de entrada..."):
        ...
        # Passando o scaler correto carregado do arquivo
        df_features_norm = normalizar_features(df_features, scaler)

    # Fazer predição
    with st.spinner("Calculando inferências..."):
        predicoes = modelo.predict(
            df_features_norm
        )  # Retorno esperado: array([[p1, p2, p3]])
        probabilidades = modelo.predict_proba(
            df_features_norm
        )  # Retorna lista de arrays por Target
    ...
    with col_r1:
        ...
        # Captura as predições de forma segura mapeando a primeira linha [0]
        pred_30dias = predicoes[0][0]

        # MultiOutputClassifier estruturação: lista_de_targets[index_target][index_linha][index_classe]
        prob_30dias = probabilidades[0][0][1]

        if pred_30dias == 1:
            st.error("**⚠️ ALTO RISCO DE REINCIDÊNCIA**")
            st.metric(
                label="Predição",
                value=f"Sim ({prob_30dias*100:.1f}% de Confiança)",
                delta=f"+{prob_30dias*100:.1f}%",
                delta_color="inverse",
            )
        else:
            st.success("**✓ Baixo Risco de Reincidência**")
            st.metric(
                label="Predição",
                value=f"Não ({(1-prob_30dias)*100:.1f}% de Confiança)",
            )

        # Barra de progresso baseada na probabilidade calculada
        st.progress(
            prob_30dias,
            text=f"Probabilidade de Reincidência: {prob_30dias*100:.1f}%",
        )
    ...
    with col_r2:
        st.info("""
            ...
            **Ação recomendada:**
            - Se ALTO RISCO: Revisar solução técnica adotada, confirmar com cliente e agendar follow-up preventivo.
            - Se BAIXO RISCO: Seguir com o fluxo de monitoramento padrão do sistema.
        """)
    ...
    # Extrai dinamicamente a probabilidade associada à classe predita pelo modelo
    prob_familia = probabilidades[1][0][int(pred_familia)]
    ...
    with col_f1:
        st.metric(
            "Classe Predita (ID)",
            f"{int(pred_familia)}",
            help="ID mapeado pelo Label Encoder da família de produto",
        )

    with col_f2:
        st.metric("Confiança", f"{prob_familia*100:.1f}%")

    with col_f3:
        st.info(
            "ℹ️ Este ID corresponde a uma das famílias de produtos do dataset.\nConsulte o mapping original do LabelEncoder para traduzir para nome da família."
        )
    ...
    # Extrai dinamicamente a probabilidade associada à classe predita pelo modelo
    prob_defeito = probabilidades[2][0][int(pred_defeito)]
    ...
    with col_d1:
        st.metric(
            "Classe Predita (ID)",
            f"{int(pred_defeito)}",
            help="ID mapeado pelo Label Encoder do tipo de defeito",
        )

    with col_d2:
        st.metric("Confiança", f"{prob_defeito*100:.1f}%")

    with col_d3:
        st.info(
            "ℹ️ Este ID corresponde a um dos defeitos do dataset.\nConsulte o mapping original do LabelEncoder para traduzir para descrição do defeito."
        )
    ...
    with st.expander(
        "🔧 Debug - Features Utilizadas (apenas para desenvolvimento)"
    ):
...
st.markdown("""
    ...
    **⚠️ Importante:**
    ...
    - Todas as features numéricas são normalizadas antes da predição utilizando parâmetros salvos da base de treino
    
    **🔄 Fluxo de Dados:**
    ```
    Dados Brutos → Engenharia de Features → Encoding → Normalização (Fit Estático) → Modelo → Predição Dinâmica
    ```
    
    Para detalhes técnicos completos, consulte [README.md](file:/README.md).
""")

st.markdown("Última atualização: Junho 2026 | Versão: 1.1")